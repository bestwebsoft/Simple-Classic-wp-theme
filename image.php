	...
	<div id="smplclssc_content">
		<div id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
			<?php while( have_posts() ) : the_post(); ?>
				<div id="nav-single">
					<span class="nav-previous"><?php previous_image_link( '%link',  '<span class="meta-nav">&larr;</span> ' . __( 'Previous', 'simpleclassic' ) ); ?></span>
					<span class="nav-next"><?php next_image_link( '%link', __( 'Next', 'simpleclassic' ) . '<span class="meta-nav">&rarr;</span>' ); ?></span>
				</div><!-- #nav-single -->
				<?php $metadata = wp_get_attachment_metadata(); ?>
				<p class="smplclssc_data-descr">
					<?php _e( 'Posted on', 'simpleclassic' ); ?> <a href="<?php the_permalink(); ?>"><?php the_date(); ?></a> 
					<?php _e( 'at', 'simpleclassic' ); ?> <a href="<?php echo wp_get_attachment_url(); ?>" target="_blank"><?php echo $metadata['width'].' &times; '.$metadata['height']; ?></a> 
					<?php _e( 'in', 'simpleclassic' ); ?> <a href = "<?php echo get_permalink( $post->post_parent ); ?>"><?php echo get_the_title( $post->post_parent ); ?></a>
